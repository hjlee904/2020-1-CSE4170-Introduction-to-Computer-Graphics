...
void main(void) {	
	...
	vec3 v_position_EC = vec3(u_ModelViewMatrix*vec4(a_position, one_f));
...
// Just to see the computed mipmap level for debugging purposes. Remove this part for a regular rendering.
	...
	if (mipmap_level < 0.5f) 
		v_shaded_color = vec4(1.0f, 1.0f, 1.0f, 1.0f);  // White
	else if (mipmap_level < 1.5f) 
		v_shaded_color = vec4(1.0f, 0.0f, 0.0f, 1.0f);  // Red
	else if (mipmap_level < 2.5f)
		v_shaded_color = vec4(0.0f, 1.0f, 0.0f, 1.0f);  // Green
	else if (mipmap_level < 3.5f)
		v_shaded_color = vec4(0.0f, 0.0f, 1.0f, 1.0f);  // Blue
	else if (mipmap_level < 4.5f)
		v_shaded_color = vec4(0.0f, 1.0f, 1.0f, 1.0f);  // Cyan
	else if (mipmap_level < 5.5f)
		v_shaded_color = vec4(1.0f, 0.0f, 1.0f, 1.0f);  // Magenta
	else if (mipmap_level < 6.5f)
		v_shaded_color = vec4(1.0f, 1.0f, 0.0f, 1.0f);  // Yellow
	else 
		v_shaded_color = vec4(0.5f, 0.5f, 0.5f, 1.0f);  // Grey
#else
// For a normal rendering ...
	...
	v_shaded_color = lighting_equation_textured(v_position_EC, v_normal_EC, base_color);
#endif
}